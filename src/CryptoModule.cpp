...
namespace CryptoModule {
    ...
    Bytes Encrypt(const Bytes& publicKey, const Bytes& plaintext) {
        // ECIES 加密方案：临时密钥 + ECDH + AES-128-GCM (128-bit 安全基线)
        ...
        // 5. 使用 AES-128-GCM 加密明文
        EVP_CIPHER_CTX_ptr ctx(EVP_CIPHER_CTX_new(), EVP_CIPHER_CTX_free);
        HANDLE_SSL_ERROR(ctx != nullptr, "Failed to create cipher context");

        // 使用 EVP_aes_128_gcm()
        HANDLE_SSL_ERROR(EVP_EncryptInit_ex(ctx.get(), EVP_aes_128_gcm(), nullptr, aesKey.data(), iv.data()) == 1,
                        "Failed to init AES-128-GCM encryption");

        Bytes ciphertext(plaintext.size() + EVP_CIPHER_block_size(EVP_aes_128_gcm()));
    ...
    Bytes Decrypt(const Bytes& privateKey, const Bytes& ciphertext) {
        ...
        // 使用 EVP_aes_128_gcm()
        HANDLE_SSL_ERROR(EVP_DecryptInit_ex(ctx.get(), EVP_aes_128_gcm(), nullptr, aesKey.data(), iv.data()) == 1,
                        "Failed to init AES-128-GCM decryption");
