...
{
    // 1. 验证服务器签名，防止连接伪基站 [cite: 181]
    if (!CryptoModule::VerifySignature(m_ask.serversigpk, challenge.dhpubS, challenge.serversigm)) {
    ...
    // 提取设备硬件指纹，透明解密本地持久化的凭证
    std::string device_pin = GetDeviceHardwareID(); 
    std::string keystore_file = "user_" + m_uid + "_keystore.dat";
    if (!m_secureManager.UnwrapAndLoadCredential(device_pin, keystore_file)) {
        throw std::runtime_error("Local Device PIN incorrect or keystore corrupted!");
    }

    // 然后再进行 kmaster 的计算：
    CryptoModule::Bytes pwBytes(password.begin(), password.end());
    CryptoModule::Bytes kmaster = m_secureManager.ComputeMasterKey(pwBytes, R);
    ...
    // 6. 生成用户确认标签 tagU = H(sharedsecret || uid || dhpubS || serversigm || dhpubU || "clientconfirm") [cite: 187]
    ...
    // 使用当场算出的私钥进行签名
    CryptoModule::Bytes sigma = CryptoModule::Sign(sigKeyPair.privateKey, sigInput);
...
bool User::FinalizeAuthentication(const ProtocolMessages::AuthConfirmation& confirmation) {
    ...
    // 协议规范: tagS = H(sharedsecret || uid || tau || dhpubS || tagU || "serverconfirm")
    ...
    expectedTagSInput.insert(expectedTagSInput.end(), m_peerDHPub.begin(), m_peerDHPub.end()); // dhpubS
    ...
    // 3. 派生最终会话密钥 sk = H(sharedsecret || dhpubS || serversigm || dhpubU || "sessionkey")
    CryptoModule::Bytes skInput = m_sharedSecret;
    skInput.insert(skInput.end(), m_peerDHPub.begin(), m_peerDHPub.end());
    skInput.insert(skInput.end(), m_serverSigM.begin(), m_serverSigM.end());
    skInput.insert(skInput.end(), m_tempDH.publicKey.begin(), m_tempDH.publicKey.end());
