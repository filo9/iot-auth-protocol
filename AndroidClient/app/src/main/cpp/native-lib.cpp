...
Java_com_filo_iotauth_MainActivity_processAuthChallenge(
        JNIEnv* env, jobject thiz, // ⚠️注意这里：把 /* this */ 改成 thiz，后面调用 Java 要用！
    ...
    // 统一释放内存 (修复了你原代码里重复 Release 的 Bug)
    env->ReleaseStringUTFChars(uid_, uid);
    env->ReleaseStringUTFChars(pwd_, pwd);
    ...
    try {
        // ==========================================================
        // 【新增】：JNI 回调 Kotlin 进行安全的 Java 层验签
        // ==========================================================
        LOGE("开始执行 Java 层验签...");
        ...
        // 2. 找到 Java 层的 MainActivity 及其验签方法
        ...
        if (verifyMethod == nullptr) {
            return env->NewStringUTF("{\"error\": \"JNI method verifySignatureInJava not found\"}");
        }

        // 3. 构建传给 Java 的参数
        jstring jPkBase64 = env->NewStringUTF(pkBase64.c_str());
        jstring jMessageBase64 = env->NewStringUTF(dhpubSStr.c_str());
