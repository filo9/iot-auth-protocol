...
void SecureRecordLayer::Initialize(const CryptoModule::Bytes& sessionKey) {
    if (sessionKey.size() != CHACHA20_KEY_LEN) {
        throw std::runtime_error("Session key must be exactly 32 bytes for ChaCha20-Poly1305.");
    }
...
CryptoModule::Bytes SecureRecordLayer::ProtectRecord(const std::string& plaintextCommand) {
    if (m_sessionKey.empty()) throw std::runtime_error("Secure record layer not initialized.");

    // 1. 构造当前发送序列号的 Nonce
    CryptoModule::Bytes nonce = ConstructNonce(m_sendSeq);

    // 2. 初始化 ChaCha20-Poly1305 加密上下文
    EVP_CIPHER_CTX_ptr ctx(EVP_CIPHER_CTX_new(), EVP_CIPHER_CTX_free);
    if (EVP_EncryptInit_ex(ctx.get(), EVP_chacha20_poly1305(), nullptr, m_sessionKey.data(), nonce.data()) != 1) {
...
std::string SecureRecordLayer::UnprotectRecord(const CryptoModule::Bytes& securePacket) {
    ...
    // 4. ChaCha20-Poly1305 解密与认证
    EVP_CIPHER_CTX_ptr ctx(EVP_CIPHER_CTX_new(), EVP_CIPHER_CTX_free);
    if (EVP_DecryptInit_ex(ctx.get(), EVP_chacha20_poly1305(), nullptr, m_sessionKey.data(), nonce.data()) != 1) {
